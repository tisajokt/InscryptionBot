...
const sidedecks: cardName[] = ["squirrel", "empty_vessel", "skeleton", "mox_crystal"];
...
const BOT_TOKEN: string = app_config.botToken;
...
const AI_SPEED: number = slow_mode ? 500 : 0;
...
function padTrim(str: string, len: number, pad: string=" "): string {
	return str.padEnd(len, pad).substring(0, len);
}
function pickRandom(arr: any[]): any {
	return arr[Math.floor(Math.random() * arr.length)];
}
function randomSelectionFrom(arr: any[], num: number): any[] {
	var out = [];
...
class SideDeck implements Drawable {
	card: Card|cardName;
	count: number;
	...
	async attackField(i: number, j: number, player: playerIndex=this.actor): Promise<number> {
		...
		if (!otherCard) {
			for (let k = 0; k < this.fieldSize; k++) {
				...
				if (burrower && burrower.sigils.has("burrower") && (!card.sigils.has("flying") || burrower.sigils.has("leaping"))) {
		...
		if (otherCard && !otherCard.sigils.has("waterborne") &&
			(otherCard.sigils.has("leaping") || !(card.sigils.has("flying") || this.isHuman(player) && this.getPlayer(player).fanUsed))) {
...
class SoloBattle extends Battle {
...
class AutoBattler implements Battler {
	battle: Battle;
	backfield: Card[];
	cardPool: cardName[];
